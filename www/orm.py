...
import aiomysql
from _mysql_exceptions import StandardError
...
class Model(dict,metaclass=ModelMetaclass):  
    ...
    def findNumber(cls,selectField,where=None,args=None):  # @NoSelf
        ...
        if where:
            sql.append('where')
            sql.append(where)
        rs=yield from select(' '.join(sql),args,1)  # @UndefinedVariable
        if len(rs)==0:
            return None
            return rs[0]['_num_']
    ...
    def update(self):
        ...
        rows=yield from execute(self.__update__,args)  # @UndefinedVariable
        if rows!=1:
            logging.warn('failed to update by primary key:affected rows:%s'%rows)
    @asyncio.coroutine
    def remove(self):
        args=[self.getValue(self.__primary_key__)]
        rows=yield from execute(self__delete__,args)  # @UndefinedVariable
        if rows!=1:
